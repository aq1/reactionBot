...
async def social_link_handler(update: Update, _):
    ...
    for entity in update.effective_message.entities:
        if entity.type not in [entity.URL, entity.TEXT_LINK]:
            continue
        link = urlparse(
            update.effective_message.text[entity.offset : entity.offset + entity.length]
        )
        ...
        if link.hostname in (
            "www.instagram.com",
            "instagram.com",
        ) and not link.path.startswith("/stories/"):
            links.append(f"https://kkinstagram.com{link.path}")
